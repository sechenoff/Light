...
| `apps/web/app/warehouse/scan/page.tsx` | Рабочий стол кладовщика v2 — страница-оркестратор: `WorkstationShell` + постоянная навигация из 5 разделов (смена/выдача/приёмка/в работе/журнал + поломки), `?tab=` в URL. Живёт внутри общего `AppShell` (решение владельца 2026-08-02), кнопки «Сообщить» здесь нет. |
...
| `apps/web/app/api/[...path]/route.ts` | Catch-all API proxy: deny-by-default гард (`proxyAuthGate`) → нормализация пути → подстановка `X-API-Key` → проксирование; connection error handling |
| `apps/web/src/lib/proxyAuthGate.ts` | Чистое решение «подписывать ли запрос ключом API»: `PUBLIC_API_ROUTES` + проверка наличия `lr_session`/`lk_session`/`Authorization` |
| `apps/api/src/__tests__/anonymousSurface.test.ts` | Сторож публичной поверхности: обход всей таблицы маршрутов Express, 401 без сессии кроме `PUBLIC_SURFACE`; сверка этого списка с `PUBLIC_API_ROUTES` прокси |
...
| `apps/api/src/services/debtReport/buildDebtReport.ts` | Отчёт по выбранным долгам: перечитывает брони из базы, группирует по клиентам (порядок — по остроте долга), считает итоги и просрочку, отбрасывает погашенные |
| `apps/api/src/services/debtReport/renderDebtReportPdf.ts` | «Реестр задолженности» A4 **альбомный** (pdfkit): сводка, группы клиентов, разбивка сумм, просрочка, пустая графа под запись ручкой, сумма прописью, подписи |
| `apps/web/src/components/finance/DebtReportModal.tsx` | Панель «Сформировать отчёт»: состав по клиентам с удалением строк, заголовок/примечание/контакты, печать и выгрузка PDF/XLSX |
| `apps/api/src/services/billService.ts` | Счета на оплату контрагентам: CRUD, нумерация по годам с ретраем P2002, снапшоты реквизитов продавца/покупателя, `prefillBillFromBooking` (строки по брони) |
| `apps/api/src/services/billDocument/renderBillPdf.ts` | Печатная форма «Счёт на оплату» (pdfkit): банк-блок 1С, таблица, итоги, сумма прописью, QR ГОСТ Р 56042-2014, подпись ИП |
| `apps/api/src/services/paymentForm.ts` | Форма оплаты брони и надбавка за безнал: `resolveSurchargePercent`, `computeSurcharge`, подписи |
| `apps/web/src/components/finance/BillEditor.tsx` | Редактор счёта на оплату (create/edit): контрагент с реквизитами, строки, итог, «Выставить и распечатать», статусы |
| `apps/api/src/services/bookingAddon.ts` | Добор со страницы брони: `searchAddonCandidates` (общий с киоском), `addAddonItems` (mode ADDON/MERGE, soft-warn конфликт + hard cap, резерв/выдача юнитов, аудит), `mergeAddonIntoMain` (влить доп-смету в основную) |
| `apps/web/src/components/bookings/AddonItemsModal.tsx` | Модалка «+ Добор» на выданной броне: поиск с доступностью на даты, корзина со степпером до `addCap`, переключатель «отдельной доп-сметой / в основную смету», карточка конфликта «под ответственность» |
...
- **Маршрут без `rolesGuard` публичен по факту** (фикс 2026-09-16). Next-прокси подставляет `X-API-Key` каждому запросу, включая анонимный, поэтому валидный ключ отличает «наш фронт» от постороннего софта, но НЕ сотрудника от анонима. До фикса так наружу уходили `GET /api/equipment*` (на проде — 295 позиций с тарифами, без входа в систему) и `POST /api/users/upsert` (анонимная запись в `User`). Гард вешается на **монтировании префикса**, а не на отдельных хендлерах: иначе следующий добавленный read-маршрут снова уедет наружу. Сторож — `apps/api/src/__tests__/anonymousSurface.test.ts`: обходит всю таблицу маршрутов Express и требует 401 без сессии от каждого, кроме явного `PUBLIC_SURFACE` (10 записей). Новый публичный маршрут = строка в этом списке, видимая на ревью.
- **Прокси не подписывает анонимные запросы** — `apps/web/src/lib/proxyAuthGate.ts` (deny-by-default). Публично: `/api/auth/*`, `/api/lk/*`, `POST /api/warehouse/auth`, `GET /api/warehouse/workers/names`. Без cookie `lr_session`/`lk_session` и без `Authorization` → 401 из прокси, не доходя до сети. Проверяется **наличие** учётных данных, не валидность: прокси не орган авторизации, решает по-прежнему API. Путь нормализуется ОДИН раз (`normalizeApiPath`) и от него же строится адрес апстрима — иначе гард и `fetch` видят разные пути: WHATWG-URL внутри `fetch` схлопывает `..` и декодирует `%2e`, и `/api/lk/%252e%252e/equipment` (Next декодирует сегменты один раз) для наивной проверки выглядел веткой портала, а на бэкенд уходил как `/api/equipment` с ключом. Сегменты `.`, `..` и со слэшем отбиваются отдельно — 400 `BAD_PROXY_PATH`.
- **Ключ веб-прокси = первый в `API_KEYS`**, и `openclaw-` первым запрещён (`deploy.sh`, `deploy-rsync.yml` падают с ошибкой). `rolesGuard` пропускает `req.botAccess` БЕЗ проверки роли, поэтому бот-ключ у веба отдал бы весь whitelist (включая запись `users/upsert`) любому, кто дотянулся до прокси.
...
- **Тулбар `/equipment` — липкая строка, а не карточка** — `sticky top-12 z-10 lg:top-0`: `top-12` — смещение под мобильную шапку `AppShell` (она сама `sticky top-0 z-20`, ровно 48 px — `h-12`), то же значение, что у `CatalogBrowser` и липкой шапки формы брони. `z-10`, чтобы не спорить с шапкой и не перекрывать скрим мобильного меню (`z-50`). Период — единственный постоянно обведённый контрол: он якорь колонок «Занято / Доступно / Статус», которые затонированы `accent-soft` и подписаны «За выбранный период». В `PeriodPopover` правки копятся в черновике и уходят по «Применить» — `datetime-local` шлёт `onChange` на каждое нажатие, и промежуточные кадры дёргали бы `/api/availability`; пресеты в самой строке применяются сразу.
...
- **`POST /api/bookings/draft` всегда создаёт ЧЕРНОВИК — в том числе при `APPROVAL_MODE=auto`.** «auto» отменяет ШАГ СОГЛАСОВАНИЯ руководителем, а не саму стадию черновика. Публикует бронь только явное действие `POST /:id/submit-for-approval` (в auto оно сразу подтверждает: доступность + резерв юнитов + снапшот сметы). До 2026-09-16 автоподтверждение стояло прямо в `/draft`, и кнопка «Сохранить черновик» публиковала бронь: склад резервировался, состав уходил из-под правок, а на карточке уже висела подтверждённая бронь. Заявку копят по позициям в течение дня (`PATCH /:id` статус не меняет) и публикуют, когда состав собран. DRAFT не входит в `BLOCKING_STATUSES` — черновик оборудование не занимает. Регрессия зафиксирована в `apps/api/src/__tests__/autoConfirm.test.ts` (a/a2) и `BookingFormCreate.test.tsx`.
- **Форма создания знает про `approvalMode`** — в auto основная кнопка называется «Создать и подтвердить →» (а не «Отправить на согласование», которое никуда не отправляет), отдельная SA-кнопка «Создать и подтвердить» не дублируется, и цепочка `submit → approve` пропускает второй шаг, если submit уже вернул `CONFIRMED` (иначе 409 `INVALID_BOOKING_STATE` — та же развилка, что на карточке брони).
...
- **Служебная оболочка и кнопка «Сообщить»** (ревизия вёрстки 2026-09-24). `AppShell` выводит без оболочки `/login`, `/crew-calculator` и весь клиентский портал `/lk`, `/lk/*` (`isStandalonePath`): у портала своя шапка `LkShell` и своя сессия, служебный сайдбар там был пустым, а `/api/auth/me` отвечал 401 на каждой странице. Хуки оболочки живут в `StaffShell`, поэтому до развилки вызывается только `usePathname`. Кнопка «Сообщить» (`FeedbackWidget`, fixed, `z-40`) скрыта на `/login`, `/lk`, `/warehouse/scan`, `/admin/scanner` (`isFeedbackWidgetHidden`); где она видна, у `<main>` есть запас `pb-20 lg:pb-24`, иначе в конце прокрутки она закрывала последние строки и кнопки. Поэтому страницам НЕ нужен свой нижний отступ под кнопку — будет двойной. Мобильная шапка — ровно `h-12`, скрим мобильного меню — `z-50` (над кнопкой). Проверка вёрстки: скриншоты 375 / 768 / 1024 / 1440 — ломается чаще всего на 768 и 1024 (появление сайдбара 224 px).
- **Позиции брони и строки сметы — в порядке каталога, а не добавления** (2026-09-24). Смета читается и бронь комплектуется категория за категорией: порядок категорий с `/equipment/manage` (`getMergedCategoryOrder`, «Транспорт» в конце), внутри — `Equipment.sortOrder`, затем имя; произвольные позиции — в конце в порядке добавления. Сортируем при ЧТЕНИИ одним хелпером `services/lineOrder.ts` (`loadLineOrdering` + `sortLinesByCatalog`, ключи `bookingItemKey` / `estimateLineKey`), а не при записи: у `BookingItem` и `EstimateLine` нет поля позиции, добор дописывает строки в конец, старые снапшоты остаются как есть, а порядок категорий может поменяться после снапшота. Все ответы с полной бронью в `routes/bookings.ts` идут через `serializeBookingOrdered`; документы сметы получают `ordering` (`loadSmetaLineOrdering` → `buildSmetaExportDocument` / `buildSmetaFromPersistedEstimate` / `buildFullSmeta`) и нумеруют строки ПОСЛЕ сортировки — № совпадает с порядком печати в PDF и XLSX; счёт и акт — через `bookingDocumentLines` (`documentExport/bookingPdf.ts`); чек-листы киоска (`getChecklistState`) и «В работе» — тоже серверные, PIN-киоск сам порядок категорий запросить не может. UI не пересортировывает, а только группирует по первому появлению (`src/lib/groupByCategory.ts`) и ставит полосу категории (`components/bookings/CategoryBand.tsx`); колонку «Категория» при полосах не дублируем. Исключение — «Состав заявки» в форме брони: состав живёт в локальном `Map selected`, поэтому ранг берётся из уже отсортированного `/api/availability` (`create/cartOrder.ts`), а сам `Map` и порядок payload не трогаются (от них зависят `formSignature` и черновик). `POST /quote` и dryRun отдают строки в порядке добавления — это данные формы, а не документ.
...
- Панель — **`z-30`**: выше неё кнопка «Сообщить» (`z-40`) и затемнение мобильного меню (`AppShell`, `z-50`); окажись панель над скримом, как более поздний элемент в DOM, её деструктивные кнопки остались бы кликабельными при открытом меню. От плавающей кнопки «Сообщить» она разведена правым отступом (~148 px), а не слоем. На мобильном кнопки идут горизонтальной лентой со скроллом — перенос в столбик съедал пол-экрана.
...
- **Матчер скорит и исходную фразу гаффера, не только AI-имя** (`findTopCandidates` в
  `equipmentMatcher.ts`, max из двух счётов). Проверка на PDF «Яндекс Книги» 2026-09-02: модель
  прочитала 54/54 строки (Opus 5 low/high и Fable 5.1 — одинаково, различия только в
  написании нормализованных имён), а матчер терял 2 позиции, которые лежат в каталоге
  дословно: «ChineVise Grip (цепной)» (модель написала «chinavise») и «Прищепка металлическая
  большая, 160 мм…» (модель перевела в «metal clamp 160mm»). Точность теряется в матчинге, а не
  в модели — усиливать модель бессмысленно, усиливать надо сопоставление с каталогом.
- **Матчер уверен только там, где сомневаться не в чем** (сверка 12 заявок ↔ сметы, 2026-09-02:
  из 369 строк 59 уверенно неверных — почти все из-за словаря, показывающего на один вариант
  «семьи»). Правила в `findTopCandidates`: (1) псевдоним на позицию с «семьёй» (соседи, отличающиеся
  только числом/размером: Автополе 100/150/235, Трубный бум D42/D48, MattBounce 8×8/12×12; для
  однословной фразы — все позиции категории с тем же первым словом: «хейзер», «страховка») даёт
  needsReview с соседями, если числа фразы или AI-имени не выбирают вариант; псевдоним с
  `usageCount ≥ 8` — домашняя конвенция, не переспрашивается; (2) два почти равных счёта
  (< 0.1) — needsReview, не угадываем; (3) вхождение целыми словами — 0.9, внутрь слова
  («расклад» ⊂ «раскладные») — 0.45. Дальше спорные строки решает AI-подбор с каталогом.
  Маршрут: «все»/«всё» во фразе при количестве 1 → количество = наличие позиции. Промпт
  разбора велит разбивать «400x + френель» на прибор и насадку.
- **Обучение словаря 2026-09-02** (`docs/slang-training-2026-09-02/`): 12 заявок гафферов из Telegram
  (02.07–04.08) сверены с xlsx-сметами сотрудника построчно (95 агентов: пары → разметка →
  сравнение → сводка → два скептика на псевдоним). На проде: 16 псевдонимов
  (перепривязки «капы»→Кабельканалы, «грипхед»→GripHead, «63 380»→Кабель 63/380, …),
  −105 авто-записей с хвостом количества («хай роллер 4»), ~20 нормализовано. Метрика по
  автосверке с эталоном (357 строк): верно 71,7 % → 75,6 %, уверенно неверно 16,0 % → 10,4 %.
  Автообучение теперь пишет псевдоним без количества (`norm(stripQuantityTokens(rawPhrase))`
  в `slangLearning.ts`); число-модель после дефиса («Джокер-800», «M-18», «Кино 4-4») количеством
  не считается — без единицы срезаются только 1–2 цифры после слова или отбитого пробелом дефиса,
  иначе Joker-800 и Joker-400 легли бы в словарь под один ключ «джокер». Дыры каталога и открытые
  решения — в `docs/slang-training-2026-09-02/README.md`.
- **AI-подбор для спорных строк** (`services/catalogPicker.ts`, `refineMatchesWithAi`): строки, где
  матчер не уверен (needsReview / unmatched), уходят модели вместе со ВСЕМ каталогом (нумерованные
  строки вместо cuid — экономия токенов; у Claude блок каталога помечен `cache_control`) и соседними
  строками заявки для контекста. Модель различает «52xt» (прибор) и «линзу для 52xt», а «нова р300 с
  софтом» разворачивает в две позиции — вторая приходит отдельным item с той же `gafferPhrase`. Выбор
  модели — `confidence: 0.95`, всё равно проходит через панель подтверждения. «В каталоге нет» —
  результат матчера остаётся как есть. Сбой модели — предупреждение в лог, ответ не ломается.
  Уверенные совпадения матчера не пересматриваются. Выключатель — `LLM_CATALOG_PICK=off`.
...
- [Сумма проекта и оплата в реестре броней](docs/features/2026-09-23-booking-payment-summary.md) — стоимость, поступления, остаток и детализация; все брони по умолчанию.
- [Человекочитаемый журнал изменений и история аккаунтов](docs/features/2026-09-19-readable-audit-log.md) — автор, время МСК, «было / стало», фильтры, запись аудита брони.
...
Сценарий: оборудование уже у клиента, гафер звонит и просит довезти ещё. Раньше
довезти можно было только через киоск (quick-add в ISSUE-сессии), а у выданной
брони состав заблокирован (`ITEMS_LOCKED_UNTIL_RETURN`). Теперь на странице брони
в статусе ISSUED — кнопка «+ Добор» (SA + WAREHOUSE): поиск по каталогу с
доступностью на даты брони, корзина с количеством и переключатель «как считать».
Логика — `services/bookingAddon.ts`, UI — `components/bookings/AddonItemsModal.tsx`.

- **`POST /api/bookings/:id/addon-items`** `{ items: [{ equipmentId, quantity }], mode: "ADDON" | "MERGE", acknowledgedConflict? }` — CONFIRMED | ISSUED, нужна MAIN-смета (`MAIN_ESTIMATE_NOT_FOUND` — у быстрой брони её нет). Только инкремент `BookingItem.quantity`, как quick-add на складе: PATCH с items пересоздаёт позиции delete+create и сносит резервы юнитов, на которых держится сверка выдачи/приёмки.
  - `mode: "ADDON"` — отдельной доп-сметой: MAIN не трогаем, ADDON пересобирается (`recomputeAddonEstimate`), клиент получает «Смета-добор» (`…/д`).
  - `mode: "MERGE"` — в основную смету: строки MAIN увеличиваются точечно (`applyAdditionsToMainEstimate`): существующая строка — то же `unitPrice` (та же договорная цена за период), новая — по правилам основной сметы; `Estimate.id` сохраняется. Ранее сделанные доборы остаются отдельным документом.
  - Доступность: soft-warn `ADDON_CONFLICT` (details — плоские поля первого конфликта + `conflicts[]`); с `acknowledgedConflict` — добавляется «под ответственность», `AddonRecord.acknowledgedConflict`, аудит. Hard cap `ADDON_OVER_STOCK` — физический склад по политике витрины (`BLOCKING_STATUSES`, архивные не занимают, потеряшки и ремонты вычтены); при подтверждённом конфликте `occupiedByOthers` не вычитается (как у `addExtraItem` после фикса 2026-08-05).
  - UNIT-позиции: юниты резервируются сразу (`BookingItemUnit`), у выданной брони переводятся в ISSUED — иначе приёмка их не покажет (RETURN-чек-лист строится по живым резервам), и единицы навсегда останутся «доступными». Потолок для UNIT — реальный пул свободных экземпляров (`listFreeUnitIds`: AVAILABLE, не в живом резерве пересекающейся брони), а не только агрегат по датам: юнит, застрявший в ISSUED у просроченной брони с чужими датами, агрегат не вычтет. Чужие резервы «под ответственность» не отдаются. `NOT_ENOUGH_UNITS`, если свободных экземпляров меньше.
  - **При активной складской сессии по брони — 409 `SCAN_SESSION_ACTIVE`** (проверка до и внутри транзакции). Приёмка по завершении переводит все живые резервы без сканов в MISSING — довезённый во время приёмки юнит попал бы в «не принято»; во время выдачи позиция добавляется в чек-листе киоска. Модалка показывает подсказку сервера («завершите или отмените сессию в киоске»).
  - Аудит `BOOKING_ADDON_ADDED` — по записи на позицию (`diffFields` выбрасывает массивы): mode, equipmentId/Name, quantity, unitsIssued, acknowledgedConflict, conflictBookingId. Финансовое событие `BOOKING_ADDON_ADDED`.
- **`POST /api/bookings/:id/addon-estimate/merge`** — влить доп-смету в основную (CONFIRMED | ISSUED | RETURNED): строки ADDON переносятся в MAIN по своему снапшоту цен, ADDON удаляется в той же транзакции (иначе финансы посчитали бы добор дважды). Сумма к оплате не меняется — меняется раскладка по документам. Аудит `BOOKING_ADDON_MERGED`. Кнопка «Влить в основную смету» — в секции «Доб-смета».
- **`GET /api/bookings/:id/addon-search?q=`** — общий с киоском `searchAddonCandidates` (`/api/warehouse/sessions/:id/addon-search` — тонкая обёртка над ним): `addCap`, `alreadyInBooking`, `conflict`, `stockTrackingMode`, `rentalRatePerShift`.
- **ADDON считается теми же правилами, что MAIN** (`resolveCatalogLinePrice` + `splitEquipmentDiscount`): `unitPrice` — за ВЕСЬ период (раньше ставка за смену → колонка «цена/смена» в PDF доб-сметы многосменной брони печаталась заниженной), договорная ставка позиции действует и на добор, процент скидки на договорные строки не начисляется.
- **Продление выданной брони не поглощает добор**: `rebuildBookingEstimate(id, { preserveAddonSplit: true })` держит количество каталожных строк MAIN в пределах старого снапшота, а `recomputeAddonEstimate` следом пересчитывает ADDON под новые смены. В остальных PATCH-правках MAIN собирается из всех позиций, и `recomputeAddonEstimate` снимает устаревший ADDON — раньше он оставался жить, и `finalAmount` считал добор дважды. `extendEndDate` вместе с `items`/`transport` в одном PATCH → 409 `ITEMS_LOCKED_UNTIL_RETURN` (иначе правка состава утекла бы в доп-смету).
- Секция «Доб-смета» читает `booking.addonEstimate` из GET /:id (раньше ходила отдельно и печатала пустые названия: API отдаёт `nameSnapshot`, компонент ждал `name`). Таблица позиций показывает «· добор ×N» и сумму строки MAIN + добор.
- Тесты: `apps/api/src/__tests__/bookingAddonItems.test.ts` (поиск, оба режима, UNIT-юниты и потолок по свободному пулу, hard cap, конфликт с подтверждением, блок при активной сессии склада, вливание обеими ролями, продление без двойного учёта), web — `AddonItemsModal.test.tsx`, `AddonEstimateSection.test.tsx`, `BookingItemsTable.test.tsx`, `BookingFinancePanel.test.tsx`.

## Форма оплаты, надбавка за безнал и счета на оплату (2026-09-16)

Две смежные вещи, оба — SUPER_ADMIN-территория денег и реквизитов.

### Форма оплаты брони: «Наличные» / «По счёту (ИП)»

- `Booking.paymentForm` (`CASH` | `CASHLESS`, default CASH), `Booking.cashlessSurchargePercent` (снапшот процента), `Booking.surchargeAmount` (посчитанная сумма надбавки). Дефолт процента — `OrganizationSettings.cashlessSurchargePercent` (9 %), правится на `/settings/organization` → «Финансовая политика».
- **Процент — снапшот на брони.** `resolveBookingSurchargePercent` (services/bookings.ts) при CASHLESS без явного процента фиксирует дефолт из настроек в момент создания/смены формы; смена дефолта в настройках старые брони не трогает. CASH обнуляет процент.
- **База надбавки — всё, что клиент платит:** оборудование после скидки (MAIN + ADDON) + транспорт. `recomputeBookingFinance` пересчитывает `surchargeAmount` и включает его в `finalAmount`; договорной итог (`manualFinalAmount`) надбавку не получает — это уже финальная сумма. Чистая математика — `services/paymentForm.ts` (`resolveSurchargePercent`, `computeSurcharge`, `surchargeLabel`).
- `/quote`, `/draft` (dryRun и запись), `PATCH /:id` (dryRun и запись, включая пересчёт `wasInReview`) принимают `paymentForm` и `cashlessSurchargePercent`, отдают `paymentForm`, `surchargePercent`, `surchargeAmount`, `grandTotal` с надбавкой.
- **Полная смета печатает надбавку отдельной строкой** «Безналичный расчёт (+9 %)» перед «ИТОГО К ОПЛАТЕ» (PDF, XLSX, ЛК): `buildFullSmeta({ surchargePercent })` → `SmetaFullExportDocument.surcharge`, `grandTotal` включает надбавку. У одиночной сметы без добора и транспорта общий блок появляется только ради надбавки.
- Web: `create/PaymentFormCard.tsx` (сегментный переключатель + процент; поле процента текстовое — в number-поле не вводится запятая; править процент может только SA), `SummaryPanel` — строка надбавки и локальный расчёт до ответа сервера, карточка брони — чип «По счёту (ИП) · +9 %» в hero и строка в «Из чего складывается сумма». Дефолтный процент форма узнаёт из ответа `/quote` (WAREHOUSE не имеет доступа к `/api/settings`).

### Счета на оплату контрагентам (`Bill`, `/finance/bills`)

Отдельный реестр от `Invoice`: тот — финансовое обязательство по броне (платежи, дебиторка), а `Bill` — печатный документ «Счёт на оплату», который ИП выставляет заказчику (бывает и без брони: монтаж, консультация). Смешивать нельзя: у `Invoice` нет строк и реквизитов контрагента, а у `Bill` — платежей.

- **Модели:** `Bill` (год + номер, `@@unique([year, number])`; статус ISSUED/PAID/CANCELLED; `sellerSnapshot`/`payerSnapshot` — JSON-снапшоты реквизитов на момент выставления; `bookingId?` с `SetNull`), `BillLine` (позиция, наименование, ед., кол-во, цена, сумма). Реквизиты контрагента живут на `Client` (`legalName`, `inn`, `kpp`, `ogrn`, `legalAddress`, `postalAddress`, банк) — форма счёта дописывает их в карточку, следующий счёт подставит сам. Реквизиты продавца для бланка — в `OrganizationSettings` (`ogrn`, `signerName`, `signerTitle`, `taxNote`).
- **Нумерация — целое число по годам**, авто «следующий за год» с ретраем на `P2002` (два одновременных «Выставить»); ручной занятый номер → 409 `BILL_NUMBER_TAKEN`. Год — по московской дате документа.
- **API** `/api/bills` (SUPER_ADMIN, router-level): `GET /` (фильтры `status`, `year`, `clientId`, `q` — поиск по имени/юримени/ИНН регистронезависимо для кириллицы фильтруется в приложении; `counts`/`sums` по статусам, `years`), `GET /next-number`, `GET /prefill?bookingId=` (объявлены ДО `/:id`), `POST /`, `GET /:id`, `PATCH /:id` (пересобирает строки, переснимает снапшоты; CANCELLED не редактируется → 409 `BILL_CANCELLED`), `POST /:id/status`, `GET /:id/pdf` (inline). Аудит `BILL_CREATE` / `BILL_UPDATE` / `BILL_STATUS`, `entityType: "Bill"`.
- **Заготовка по брони** (`prefillBillFromBooking`) — зеркало «Из чего складывается сумма»: строка аренды (MAIN + ADDON после скидки) + транспорт + «Безналичный расчёт, +N %»; при договорном итоге — одна строка на всю сумму. `expectedTotal` = `finalAmount`; форма предупреждает, если строки дают другую сумму.
- **Печатная форма** `services/billDocument/renderBillPdf.ts` — концепт A дизайн-панели («Канон 1С, доведённый», `docs/mockups/bill-invoice/`, судьи 127.5 / 113 / 106.5): банк-блок «как в платёжке» сверху, заголовок с датой словами + микро-сводка, Поставщик/Покупатель/Основание строками, таблица без клеток, итоги правым столбиком с пометкой о налоге, плашка «ВСЕГО К ОПЛАТЕ», «Сумма прописью» (`utils/amountInWords.ts`), назначение платежа в рамке (≤ 210 знаков, «Без НДС» в конце), QR по ГОСТ Р 56042-2014 (`ST00012|Name=…|PersonalAcc=…|Sum=<копейки>|Purpose=…`, bwip-js `qrcode`, 30×30 мм) и подпись ИП + «М.П.». Один набор банковских реквизитов на листе (банк покупателя не печатается). `fitOneLine` ужимает кегль вместо переноса в ячейках: pdfkit при заданной `width` переносит строку даже с `lineBreak: false`. Футер «Стр. N из M» — только при переносе.
- Web: `/finance/bills` (реестр, вкладка «Счета на оплату»; старая вкладка «Счета» переименована в «Счета по броням»), `/finance/bills/new?bookingId=` и `/finance/bills/[id]` — общий `components/finance/BillEditor.tsx` («Выставить и распечатать» создаёт счёт и сразу печатает PDF через `printEstimate` из `estimateExport.ts`, у которого теперь настраиваемое сообщение о 404). Кнопка «Счёт на оплату» на карточке брони (CONFIRMED/ISSUED/RETURNED, SA).
- Тесты: `apps/api/src/__tests__/paymentSurcharge.test.ts`, `bills.test.ts`, `amountInWords.test.ts`; web — `PaymentFormCard.test.tsx`, `SummaryPanelSurcharge.test.tsx`, `BillEditor.test.tsx`.

## Взыскание долгов: роль COLLECTOR и отчёт по выбранным долгам (2026-09-16)

У проката появился сотрудник, который занимается только возвратом долгов. Ему
нужен свой вход и печатный документ обзвона — но ни выручки, ни броней, ни
каталога он видеть не должен.

### Роль `COLLECTOR` («Взыскание») — четвёртая роль

- `UserRole` в Prisma получил значение `COLLECTOR`. На SQLite enum — это TEXT,
  поэтому `prisma db push` в деплое данные не трогает.
- **Доступ ровно к четырём ручкам:** `GET /api/finance/debts`,
  `GET /api/finance/debts.xlsx`, `POST /api/finance/debts/report.pdf`,
  `POST /api/finance/debts/report.xlsx` — через `debtsAccess =
  rolesGuard(["SUPER_ADMIN", "COLLECTOR"])` в `routes/finance.ts`. Всё
  остальное в `financeRouter` осталось `superAdminOnly`; напоминания
  (`draft-reminder`, `mark-reminded`, `remindable`) и клиентские выписки
  `:clientId/report.pdf` роли взыскания НЕ даны — они пишут в базу или тратят AI.
- **Ничего не меняет.** Платежи, списание долга, правка броней закрыты на
  сервере; в UI строки реестра у этой роли показывают «только просмотр» вместо
  кнопок, и на десктопе, и на мобильном.
- **Домашняя страница — сам реестр.** `homeForRole(role)` в `roleMatrix.ts`:
  у взыскания это `/finance/debts`, у остальных `/day`. `useRequireRole`
  редиректит отказ туда же, а не жёстко на `/day` — иначе роль попадала бы
  на пустой экран. `/day` для `COLLECTOR` — редирект (`DayCollectorRedirect`).
- `menuByRole.COLLECTOR` — одна секция «Взыскание» с единственным пунктом;
  `FinanceTabNav` показывает ей одну вкладку. Матрица прав `/admin/roles`
  получила четвёртую колонку: `MatrixRow.collector` **опционально**, не задано =
  «нет» (перечислять сорок запретов построчно — шум).

### Отчёт по выбранным долгам

- **Выбор живёт в реестре, а не в отдельном экране.** Чекбоксы в строках
  `/finance/debts` + «отметить все в текущем фильтре» в шапке. Выбор **не
  сбрасывается при смене фильтра** — в этом весь смысл: руководитель проходит
  по клиентам фильтр за фильтром и накапливает один документ. Липкая панель
  внизу показывает «выбрано N · M клиентов · сумма» и сколько из выбранного
  сейчас скрыто фильтром. Выбор подрезается только когда долг ушёл из выдачи
  (погашен) — иначе счётчик врал бы.
- **`POST /api/finance/debts/report.{pdf,xlsx}`**, а не GET: список броней
  бывает на десятки позиций и в query-строку не помещается. Тело —
  `{ bookingIds (1…500), title?, note?, includeContacts? }`. Маршруты объявлены
  рядом с остальными debts-ручками; пересечения с `/finance/debts/:clientId/...`
  нет (там два сегмента после «debts», здесь один).
- **Данные перечитываются из базы**, а не приходят с клиента: между открытием
  страницы и нажатием кнопки долг мог быть погашен. Погашенные и отменённые
  брони выпадают в `skipped`; если не осталось ничего — 400
  `DEBT_REPORT_NOTHING_TO_COLLECT`, а не пустой документ.
- **Документ — A4 альбомный.** Девять колонок, одна из которых пустая графа
  «Дата контакта / результат» под запись ручкой; в портрете пришлось бы
  выбросить разбивку «выставлено / оплачено / остаток». Клиенты идут по остроте
  долга (сначала те, у кого просрочено больше денег), внутри клиента — от самого
  старого долга. Долги старше 60 дней (`OVER_AGED_DAYS`) подсвечены. Внизу —
  сумма прописью (общий `utils/amountInWords.ts`) и подписи «Отчёт составил /
  Принял к взысканию».
- Контакты клиента печатаются **по флажку** и по умолчанию выключены: у
  сотрудника они и так есть на экране, а бумага с телефонами клиентов гуляет
  по офису.
- **Отсеянное не проглатывается молча.** Причина различается по каждой брони
  (`SkipReason`: `PAID` / `CANCELLED` / `ARCHIVED` / `NOT_FOUND`) — архивная
  бронь с живым долгом это не «погашено», и писать так значило бы потерять долг
  из виду. Тело ответа — сам файл, поэтому сводка уходит заголовком
  `X-Export-Notice` (percent-encoded), а `estimateExport.ts` всплывает им тостом:
  «2 долга не попали в отчёт: 1 — уже погашен, 1 — отменён». Любой будущий
  экспорт может пользоваться тем же заголовком.
- **Не путать с `/finance/debts/:clientId/report.pdf`** — тот документ
  клиентский («вот ваш долг, оплатите»), один клиент, портрет. Этот —
  внутренний рабочий реестр обзвона на несколько клиентов. Объединять их не надо.

### Попутные фиксы

- **`GET /api/finance/debts.xlsx` всегда падал с 500.** В `Content-Disposition`
  кириллица стояла в ASCII-части `filename="Дебиторка_…"`, а Node отвергает
  не-latin1 в значении заголовка (`ERR_INVALID_CHAR`) — кнопка «Экспорт XLSX» на
  странице долгов не работала никогда. Теперь через общий
  `buildAttachmentContentDisposition` (транслитерация + `filename*=UTF-8''`).
  Нашёл тест новой роли, который просто дёрнул этот маршрут.
- **`/bookings`, `/calendar`, `/equipment` получили `useRequireRole`** — раньше
  по прямому URL любая роль открывала каркас страницы с красной ошибкой
  «Доступ запрещён по роли» (данные API не отдаёт, но выглядит как поломка).
  Списки ролей — зеркало `menuByRole`.
- **Счёт на оплату (`renderBillPdf.ts`) страдал тем же, что и новый отчёт:
  ненулевые поля документа.** pdfkit при заданных `margins` добавляет страницу
  САМ, как только текст доходит до нижнего поля, — параллельно с нашей ручной
  пагинацией; выходили полупустые листы (36 строк реестра разъезжались на 9
  страниц вместо 4). Правило простое и уже действовало в смете: **ручная
  пагинация = нулевые поля в конструкторе `PDFDocument` + свои отступы**.
  Попутно блок подписи в счёте резервируется ВМЕСТЕ с зоной оплаты
  (`SIGNATURE_H`): по отдельности у счёта на 30 позиций зона влезала в остаток
  страницы, а подпись — уже нет, и последний лист выходил пустым с одной
  строчкой «ИП ____ (Фамилия)».

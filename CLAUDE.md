...
- **Экран входа киоска закрыт API-ключом.** `warehousePublicRouter` висел ДО `apiKeyAuth`, поэтому `GET /api/warehouse/workers/names` отдавал имена всех кладовщиков любому в интернете, а `POST /api/warehouse/auth` позволял анонимно перебирать четырёхзначный PIN. «Публичный» здесь означает «без Bearer-токена склада», а не «доступный всем»: роутер перенесён за `apiKeyAuth`, киоск ходит через Next-прокси с `X-API-Key` и работает как раньше.

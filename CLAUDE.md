...
- **`POST /api/bookings/:id/addon-items`** `{ items: [{ equipmentId, quantity }], mode: "ADDON" | "MERGE", acknowledgedConflict? }` — CONFIRMED | ISSUED, нужна MAIN-смета (`MAIN_ESTIMATE_NOT_FOUND` — у быстрой брони её нет). Только инкремент `BookingItem.quantity`, как quick-add на складе: PATCH с items пересоздаёт позиции delete+create и сносит резервы юнитов, на которых держится сверка выдачи/приёмки.
  ...
  - UNIT-позиции: юниты резервируются сразу (`BookingItemUnit`), у выданной брони переводятся в ISSUED — иначе приёмка их не покажет (RETURN-чек-лист строится по живым резервам), и единицы навсегда останутся «доступными». `NOT_ENOUGH_UNITS`, если свободных экземпляров меньше.
...
- Тесты: `apps/api/src/__tests__/bookingAddonItems.test.ts` (поиск, оба режима, UNIT-юниты, hard cap, конфликт с подтверждением, вливание, продление без двойного учёта), web — `AddonItemsModal.test.tsx`, `AddonEstimateSection.test.tsx`.

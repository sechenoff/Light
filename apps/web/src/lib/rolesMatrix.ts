...
export const TECH_NOTES: TechNote[] = [
  ...
  { text: "**Бот-ключи.** `botScopeGuard` (ограничивает ключи `openclaw-*`) остаётся слоем над `rolesGuard`. Бот-ключ пропускается через `req.botAccess === true`, роль не проверяется." },

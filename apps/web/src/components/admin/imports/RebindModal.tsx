...
export function RebindModal({ sourceName, currentEquipmentId, onRebind, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setLoading(true);
      try {
        type EquipmentResponse = { items?: { id: string; name: string; category: string; brand: string | null; rentalRatePerShift: string }[] };
        const data = await apiFetch<EquipmentResponse | { id: string; name: string; category: string; brand: string | null; rentalRatePerShift: string }[]>(
        ...
        setResults(items.map((e) => ({
          ...
          category: e.category,
          brand: e.brand,
          rentalRatePerShift: e.rentalRatePerShift,
        })));
      } catch {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {isCurrent && (
                    <p className="text-[11px] text-emerald">Текущая связь</p>
                  )}
                </div>

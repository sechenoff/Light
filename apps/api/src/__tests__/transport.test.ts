...
describe("POST /api/bookings/quote — регрессия без транспорта", () => {
  it("без transport — отвечает как раньше, grandTotal = equipmentTotal", async () => {
    ...
    // Multi-vehicle: transport — массив (пустой когда транспорта нет)
...
describe("POST /api/bookings/quote — с транспортом", () => {
  it("grandTotal = equipmentTotal + transport.total", async () => {
    ...
    expect(res.body.transport).toHaveLength(1);
    expect(res.body.transport[0].vehicleName).toBe("Ford");
    ...
    const transportTotal = Number(res.body.transport[0].total);
    const grandTotal = Number(res.body.grandTotal);
    expect(grandTotal).toBeCloseTo(eqTotal + transportTotal, 2);
    expect(Number(res.body.transportSubtotal)).toBeCloseTo(transportTotal, 2);
  });
...
describe("POST /api/bookings/draft — сохраняет транспорт (multi-vehicle)", () => {
  it("draft с транспортом создаёт BookingVehicle и пишет transportSubtotalRub", async () => {
    const body = {
    ...
    // Новые брони используют BookingVehicle, legacy vehicleId остаётся null
    ...
    // transportSubtotalRub + per-row subtotal должны соответствовать ford shiftPrice
    const fordInDb = await prisma.vehicle.findUnique({ where: { id: fordId } });
    const fordRate = Number(fordInDb!.shiftPriceRub);
    expect(Number(saved!.transportSubtotalRub)).toBeCloseTo(fordRate, 2);
    expect(Number(saved!.vehicles[0].subtotalRub)).toBeCloseTo(fordRate, 2);
  });
});

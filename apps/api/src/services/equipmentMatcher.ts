...
export function stripQuantityTokens(s: string): string {
  return s
    ...
    // Дефис перед количеством: «Капа — 12 шт.», «Капы - 8ш», «Мотыль-6», «Стойка - 2».
    // Без единицы срезаем только 1–2 цифры после слова или после отбитого пробелом
    // дефиса: «Joker-800», «M-18», «Кино 4-4», «Автополе 235 - 410» — дефис внутри
    // модели, число здесь мощность или размер. Срезать его значило бы записать в
    // словарь «джокер» и уравнять Joker-800 с Joker-400.
    .replace(/[-—–]\s*\d+\s*(?:шт\.?|штук|ш|pcs)\s*\.?\s*$/i, " ")
    .replace(/(?<=[a-zа-яё]{3})[-—–]\s*\d{1,2}\s*\.?\s*$/i, " ")

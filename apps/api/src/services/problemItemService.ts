 ...
 * «Найдено» вернуло бы в оборот то, что инвентаризация сочтёт излишком. Поэтому
 * обе операции по посчитанной позиции отправляются в инвентаризацию: 409
 * STOCK_COUNT_LINE_COUNTED. Непосчитанные строки не мешают — их ожидание
 * берётся живым и новую потеряшку увидит.
 */
...
export async function resolveProblemItem(
  ...
  return prisma.$transaction(async (tx: TxClient) => {
    ...
    // «Найдено» у безъюнитной карточки возвращает количество в «на полке
    // должно быть». Для посчитанной позиции это решает инвентаризация
    // («Нашлось» в строке) — иначе излишек закрыл бы потеряшку дважды.
    // «Не найдено» доступность не меняет (карточка и так вычитается) — можно.
    const equipmentId = pi.equipmentId ?? pi.bookingItem?.equipmentId ?? null;
    if (outcome === "FOUND" && !pi.equipmentUnitId && equipmentId) {
      await assertNotCountedInOpenStockCount(tx, equipmentId, "решите там («Нашлось»)");
    }
